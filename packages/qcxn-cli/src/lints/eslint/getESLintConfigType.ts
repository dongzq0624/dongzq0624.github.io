 ...
 * @param cwd
 * @param pkg
 * @returns qcxn-eslint-config/index
 * @returns qcxn-eslint-config/react
 * @returns qcxn-eslint-config/typescript/index
 * @returns qcxn-eslint-config/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return 'qcxn-eslint-config/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '');
}
